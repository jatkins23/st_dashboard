...
class CLIPEncoder:
    ...
    def encode_pil_images(self, images: Sequence[Image.Image]) -> np.ndarray:
        if not images:
...
if __name__ == "__main__":
    ...
    if args.folder:
        ...
        pairs = clip_encoder.encode_folder(args.folder)
    ...
    if args.image:
        vec = clip_encoder.encode_image(args.image)
        print(f"image embedding shape: {vec.shape}, norm={np.linalg.norm(vec):.6f}")

    if args.text:
        vec = clip_encoder.encode_text(args.text)
